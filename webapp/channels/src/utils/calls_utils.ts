...
export const openKmeetInExternalWindow = (baseUrl: string, jwt: string, subject: string) => {
    window.open(baseUrl + `?jwt=${jwt}#config.localSubject="${subject}"#config.prejoinConfig.enabled=false&config.deeplinking.disabled=true&config.startWithVideoMuted=true`, '_blank', 'noopener');
};
