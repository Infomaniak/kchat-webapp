...
export default class LoggedIn extends React.PureComponent<Props> {
    ...
    private setCallListeners() {
        if (isDesktopApp() && isServerVersionGreaterThanOrEqualTo(getDesktopVersion(), '2.3.0')) {
            window?.callManager?.onCallJoined?.((_, message) => this.props.actions.joinCall(message.channelId));
            window?.callManager?.onCallDeclined?.((_, message) => this.props.actions.declineCall(message.channelId));
            window?.callManager?.onCallCancel?.((_, message) => this.props.actions.cancelCall(message.channelId));
        }
    }
