...
describe('components/integrations/InstalledOutgoingWebhooks', () => {
    ...
    test('should include outgoing webhooks when filter matches creator username', () => {
        const wrapper = shallow<InstalledOutgoingWebhooks>(
            <InstalledOutgoingWebhooks {...defaultProps}/>,
        );

        expect(wrapper.instance().outgoingWebhooks('sudheer').length).toBe(outgoingWebhooks.length);

        expect(wrapper.instance().outgoingWebhooks('no-match').length).toBe(0);
    });
});
