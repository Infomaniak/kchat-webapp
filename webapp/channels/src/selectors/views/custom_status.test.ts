...
import {makeGetCustomStatus, getRecentCustomStatuses, isCustomStatusEnabled, showPostHeaderUpdateStatusButton, showStatusDropdownPulsatingDot} from 'selectors/views/custom_status';
...
describe('getCustomStatus', () => {
    ...
    it('should return customStatus object when there is custom status set', async () => {
        const store = await configureStore();
        const newUser = {...user};
        newUser.props.customStatus = customStatus as any;
...
describe('getRecentCustomStatuses', () => {
    ...
    it('should return empty arr if there are no recent custom statuses', async () => {
        const store = await configureStore();
        jest.spyOn(PreferenceSelectors, 'get').mockReturnValue(preference.myPreference.value);
