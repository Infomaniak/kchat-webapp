...
describe('Utils.isEmail', () => {
    const testCases = [
        ...
        {email: '#!$%&\'*+-/=?^_`{}|~@example.org', valid: true},
        {email: 'example@s.solutions', valid: true},
        {email: 'Abc.example.com', valid: false},
        {email: 'A@b@c@example.com', valid: false},
        {email: '<testing> test.email@example.com', valid: false},
        {email: 'test <test@address.do>', valid: false},
        {email: 'comma@domain.com, separated@domain.com', valid: false},
        {email: 'comma@domain.com,separated@domain.com', valid: false},
    ];

    for (const {email, valid} of testCases) {
        it(`should return ${valid} for "${email}"`, () => {
