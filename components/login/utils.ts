...
export function getChallengeAndRedirectToLogin() {
    ...
    generateCodeChallenge(codeVerifier).then((challenge) => {
        ...
        window.location.assign(`${IKConstants.LOGIN_URL}authorize?access_type=offline&code_challenge=${codeChallenge}&code_challenge_method=S256&client_id=${IKConstants.CLIENT_ID}&response_type=code`);
    }).catch(() => {
